        ...
        for result in results:
            ...
            if boxes is not None and len(boxes.xyxy) > 0:
                ...
                if high_confidence_indices.size > 0:
                    ...
                    self.queue.put(x_center)
                    self.queue.put(y_center)
    ...
    def move(self):
        while not self.spot:
            if self.is_reft_mouse_button_pressed():
                x_center = self.queue.get()
                y_center = self.queue.get()
                screen_width, screen_height = pyautogui.size()
                center_x = screen_width // 2
                center_y = screen_height // 2
                # 打印目标坐标
                print('目标坐标:', x_center, y_center)

                # 计算目标的相对坐标
                delta_x = (x_center - center_x) * self.sx
                delta_y = (y_center - center_y) * self.sy
